...
- `paperless-ngx-data` (5Gi) - Configuración y base de datos SQLite cache
...
2. Verificar conexión a Redis:
   ```bash
   kubectl exec -n paperless deployment/paperless-ngx -c paperless-ngx -- \
     python3 -c "import redis; r=redis.from_url('redis://:PASSWORD@192.168.4.203:6379'); print(r.ping())"
   ```
...
1. Verificar que PostgreSQL está accesible:
   ```bash
   kubectl exec -n paperless deployment/paperless-ngx -c paperless-ngx -- \
     python3 -c "import psycopg2; conn=psycopg2.connect(host='192.168.4.204', dbname='paperless', user='paperless', password='$(tofu output -raw paperless_db_password 2>/dev/null || echo 'CHECK_SECRET')'); print('DB OK')"
   ```
...
- [Feature Spec: 027-paperless-ngx](../specs/027-paperless-ngx/spec.md)
