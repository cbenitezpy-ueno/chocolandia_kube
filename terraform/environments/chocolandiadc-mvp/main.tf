# ChocolandiaDC MVP - 3-Node K3s Cluster on Eero Network (HA)
# Deploys master1 + nodo03 (control-planes) + nodo1 (worker) using k3s-node module
...
# ============================================================================
# Additional Control Plane Node (HA) - Server Role
# ============================================================================
...
module "nodo03" {
  ...
  # Node identity
  hostname  = var.nodo03_hostname
  node_ip   = var.nodo03_ip
  node_role = "server"
  ...
  # Cluster join configuration (from master1) - HA mode
  server_url = module.master1.server_url
  join_token = module.master1.cluster_token
...
# Wait for all nodes to be Ready before completing
